...
export default function OnboardingTerms() {
  ...
  return (
    <AuthScaffold
      ...
      <View>
        <Text className="font-sans-bold text-title-1 text-foreground">Almost done</Text>
        <Text className="mt-2 font-sans text-body text-ink-secondary">
          Please review and accept to finish.
        </Text>
      </View>
      ...
      <View className="mt-10">
        <Pressable
          ...
          className="border-hairline min-h-touch-lg flex-row items-start gap-3 rounded-lg border bg-card p-4">
          <View
            className={`mt-0.5 h-7 w-7 items-center justify-center rounded-sm border-2 ${
              termsAccepted ? 'border-primary bg-primary' : 'border-strong bg-card'
            }`}>
            {termsAccepted ? <Ionicons name="checkmark" size={20} color="#FFFFFF" /> : null}
          </View>
          <View className="flex-1 flex-row flex-wrap items-center">
            <Text className="font-sans text-body text-foreground">I agree to the </Text>
            <Link label="Terms of Service" size="md" onPress={() => router.push('/legal/terms')} />
            <Text className="font-sans text-body text-foreground"> and </Text>
