...
export async function GET(req: Request) {
  ...
  const csv = toCsv([
    ['audit_id', 'user_id', 'action', 'resource_type', 'resource_id', 'ip_address', 'created_at'],
    ...rows.map((r) => [
      r.id,
      r.user_id ?? '',
      r.action,
  ...
  return new NextResponse(csv, {
    ...
    headers: {
      ...
      'content-disposition': `attachment; filename="compliance-audit-${range.startLabel}_${range.endLabel}.csv"`,
    },
  });
}
