...
//   1. sessionClaims.role  — populated by Clerk's JWT template that
//      mirrors `public_metadata.role`. No Clerk API call.
//   2. publicMetadata.role — fetched via Clerk's currentUser(). One API
//      call, cached per request via React's cache().
//
// The DB column `users.role` (see packages/shared/src/db/schema.ts) is
// the canonical write target; Clerk's publicMetadata mirrors it via the
// user account webhook (apps/web/app/api/webhooks/clerk/route.ts). For
// row-level enforcement, see Supabase RLS policies — never trust this
// helper alone.
 ...
 *
 * Returns `null` for unauthenticated callers, or for signed-in users
 * whose role hasn't been assigned. Wrapped in React's cache() so
 * concurrent server components within a single request share one
 * resolution.
 */
