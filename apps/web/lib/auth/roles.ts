// Server-side RBAC helpers for apps/web.
//
// Three-tier enforcement (defense in depth):
//
//   1. Server layer (this file + current-user.ts): role check inside server
...
//      enforce access at the row level. The real boundary.
//
 ...
 *
 * Hits Clerk's user API. Use sparingly — for the current session, prefer
 * {@link currentUserHasRole}, which uses the cached `getCurrentUserWithRole`.
 */
export async function hasRole(clerkUserId: string, role: UserRole): Promise<boolean> {
  try {
    ...
    // Fail closed if Clerk is unavailable.
    return false;
  }
}
 ...
 *
 * Reads from the cached `getCurrentUserWithRole`, so calling this multiple
 * times within a single request is cheap.
 */
...
/**
 * Read the current session's userId without full role resolution. Useful
 * for routes that just need to know "is anyone signed in".
 */
